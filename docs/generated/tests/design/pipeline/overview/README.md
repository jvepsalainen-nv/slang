...
| A non-trivial source successfully traverses lex, preprocess, parse, check, lower, IR passes, and emit to produce SPIR-V text.             | functional, expansion | [#end-to-end-flow](../../../design/pipeline/overview.md#end-to-end-flow) | [`end-to-end-flow-spirv.slang`](end-to-end-flow-spirv.slang), [`end-to-end-flow-spirv-hlsl.slang`](end-to-end-flow-spirv-hlsl.slang), [`end-to-end-flow-spirv-glsl.slang`](end-to-end-flow-spirv-glsl.slang), [`end-to-end-flow-spirv-metal.slang`](end-to-end-flow-spirv-metal.slang), [`end-to-end-flow-spirv-wgsl.slang`](end-to-end-flow-spirv-wgsl.slang)                           |
| A non-trivial source successfully traverses the full pipeline to produce HLSL text with the entry-point body intact.                      | functional, expansion | [#end-to-end-flow](../../../design/pipeline/overview.md#end-to-end-flow) | [`end-to-end-flow-hlsl.slang`](end-to-end-flow-hlsl.slang), [`end-to-end-flow-hlsl-glsl.slang`](end-to-end-flow-hlsl-glsl.slang), [`end-to-end-flow-hlsl-spirv.slang`](end-to-end-flow-hlsl-spirv.slang), [`end-to-end-flow-hlsl-metal.slang`](end-to-end-flow-hlsl-metal.slang), [`end-to-end-flow-hlsl-wgsl.slang`](end-to-end-flow-hlsl-wgsl.slang)                             |
...
| [#end-to-end-flow](../../../design/pipeline/overview.md#end-to-end-flow)                   | missing-example       | The `## End-to-end flow` section asserts the source reaches a "target artefact" via emit but names the back-ends only generically; it does not enumerate the per-target form each compute entry point takes in the emitted text (HLSL `numthreads`, GLSL `layout(local_size_x...)`, SPIR-V `OpExecutionMode ... LocalSize`, Metal entry function, WGSL `@compute`/`@workgroup_size`). The fan-out tests pin each form, but the doc gives the reader no concrete per-target shape. | Add a short per-target table to `## End-to-end flow` mapping a `[shader("compute")]` entry point to the concrete compute marker each text back-end emits (`numthreads`, `layout(local_size_x...)`, `OpExecutionMode LocalSize`, Metal kernel function, `@compute`/`@workgroup_size`). |
