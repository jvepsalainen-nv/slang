...
generated_at: 2026-06-11T11:16:03+00:00
...
| C22 — Variable declared in for-loop init is scoped to the for statement; can be redeclared in successive for loops.                    | functional, expansion | [#for-statement](../../../../language-reference/statements.md#for-statement)                           | [`for-init-scope-functional.slang`](for-init-scope-functional.slang), [`for-init-scope-emission-glsl.slang`](for-init-scope-emission-glsl.slang), [`for-init-scope-emission-metal.slang`](for-init-scope-emission-metal.slang), [`for-init-scope-emission-spirv.slang`](for-init-scope-emission-spirv.slang), [`for-init-scope-emission-wgsl.slang`](for-init-scope-emission-wgsl.slang)                               |
...
| C39 — `discard` in a fragment shader emits a discard/kill instruction in HLSL and SPIR-V.                                              | functional, expansion | [#discard-statement](../../../../language-reference/statements.md#discard-statement)                   | [`discard-emission.slang`](discard-emission.slang), [`discard-emission-glsl.slang`](discard-emission-glsl.slang), [`discard-emission-metal.slang`](discard-emission-metal.slang), [`discard-emission-wgsl.slang`](discard-emission-wgsl.slang)                                                 |
...
| C44 — `$for` is fully unrolled in emitted HLSL; no loop structure remains.                                                             | functional, expansion | [#compile-time-for-statement](../../../../language-reference/statements.md#compile-time-for-statement) | [`compile-time-for-emission.slang`](compile-time-for-emission.slang), [`compile-time-for-emission-glsl.slang`](compile-time-for-emission-glsl.slang), [`compile-time-for-emission-metal.slang`](compile-time-for-emission-metal.slang), [`compile-time-for-emission-spirv.slang`](compile-time-for-emission-spirv.slang), [`compile-time-for-emission-wgsl.slang`](compile-time-for-emission-wgsl.slang)                               |
...
| [#discard-statement](../../../../language-reference/statements.md#discard-statement) | missing-example | The Discard Statement section says `discard` ends fragment processing but does not enumerate the per-target emitted form (HLSL/SPIR-V `discard`/`OpKill`, GLSL `discard;`, Metal `discard_fragment()`, WGSL `discard;`); a reader cannot tell what each target emits. | Add a per-target emission table to the Discard Statement section listing the keyword/intrinsic each backend produces (`discard`, `OpKill`/`OpTerminateInvocation`, `discard_fragment()`). |
| [#compile-time-for-statement](../../../../language-reference/statements.md#compile-time-for-statement) | missing-example | The Compile-Time For section states the body is instantiated per value but does not say it is *fully unrolled with no loop structure* in every target backend, nor give the per-target emitted shape (GLSL/Metal/WGSL straight-line stores, SPIR-V no `OpLoopMerge`). | Add a note that `$for` emits as fully-unrolled straight-line code on all text targets, with a one-line example showing the repeated body and the absence of any loop construct. |
| [#for-statement](../../../../language-reference/statements.md#for-statement) | missing-example | The For Statement section states the init variable's scope is limited to the for statement but does not show the per-target emitted form proving two successive same-named init variables compile to two independent loops. | Add a note that an init-declared variable is emitted as a loop-local on every backend, illustrated by two successive `for(int i...)` loops emitting as two separate loop structures. |
