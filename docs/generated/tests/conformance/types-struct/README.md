...
| C6 — Non-static fields are read and written via the dot operator on an instance.                                            | functional, expansion | [#description](../../../../language-reference/types-struct.md#description)                                               | [`struct-basic-fields-functional.slang`](struct-basic-fields-functional.slang), [`struct-basic-fields-emission.slang`](struct-basic-fields-emission.slang), [`struct-basic-fields-emission-spirv.slang`](struct-basic-fields-emission-spirv.slang) |
| C7 — Static data member allocated from global storage; emitted as a module-scope static in HLSL.                            | functional, expansion | [#description](../../../../language-reference/types-struct.md#description)                                               | [`struct-static-member-functional.slang`](struct-static-member-functional.slang), [`struct-static-member-functional-spirv.slang`](struct-static-member-functional-spirv.slang)                                                                           |
...
| C12 — Members declared `public` are accessible; struct can be declared `public` to allow `public` members.                  | functional, expansion | [#description](../../../../language-reference/types-struct.md#description)                                               | [`struct-access-visibility-default-internal.slang`](struct-access-visibility-default-internal.slang), [`struct-access-visibility-default-internal-spirv.slang`](struct-access-visibility-default-internal-spirv.slang)                                                       |
...
| C24/C25 — Static member function invoked without an object; emits as a free function in HLSL.                               | functional, expansion | [#static-member-function](../../../../language-reference/types-struct.md#static-member-function)                         | [`struct-static-member-function.slang`](struct-static-member-function.slang), [`struct-static-member-function-spirv.slang`](struct-static-member-function-spirv.slang)                                                                               |
...
| C31 — A `property` with only a `get` accessor is read-only; the getter emits correctly in HLSL.                             | functional, expansion | [#property](../../../../language-reference/types-struct.md#property)                                                     | [`struct-property-readonly.slang`](struct-property-readonly.slang), [`struct-property-readonly-spirv.slang`](struct-property-readonly-spirv.slang)                                                                                         |
...
| C41 — Natural layout: non-static fields emitted in declaration order in HLSL struct type.                                   | functional, expansion | [#natural-layout](../../../../language-reference/types-struct.md#natural-layout)                                         | [`struct-natural-layout-emission.slang`](struct-natural-layout-emission.slang), [`struct-natural-layout-emission-spirv.slang`](struct-natural-layout-emission-spirv.slang)                                                                             |
...
| ------------------------------------------------------------------------------------------ | --------------------- | ------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A no-body struct declaration (forward declaration) declares a struct type's existence and allows it to be used in type expressions before the full definition. | unsupported-on-target | [#description](../../../../language-reference/types-struct.md#description)                                               | The forward-declaration pattern (`struct Foo; struct Foo { int x; }`) is rejected with E30200 "conflicting declaration" / E39999 "ambiguous reference" on **every** target including the existing `struct-no-body-decl.slang` HLSL test. Absent shader targets glsl metal spirv-asm wgsl (and hlsl cuda cpp) all reject it; the diagnostic fires in the front-end before any back-end runs. This contradicts the doc, which states a no-body declaration enables later use; recorded as finding `types-struct-no-body-forward-decl-conflict` and the existing HLSL test is added to expected-failures. No clean-emission test is feasible on any shader target for this input.                                                                           |
...
| [#description](../../../../language-reference/types-struct.md#description)                               | drift-from-source | The Description section states a *no-body* declaration "specifies an existence of a structure type" and "enables its use in type expressions without the member declarations", implying a separate *with-members* definition of the same name may follow. The compiler instead rejects the pair `struct Foo; struct Foo { int x; }` with `error[E30200]: conflicting declaration` / `error[E39999]: ambiguous reference` on every target (order-independent). Recorded as finding `types-struct-no-body-forward-decl-conflict`. | Either document that a same-module no-body declaration may not be completed by a later with-members definition (forward declaration is only meaningful for the `extern struct` link-time form), or describe the diagnostic that fires; do not present the plain no-body form as a general forward-declaration mechanism if the compiler forbids completing it. |

## Bootstrap test issues observed

`struct-no-body-decl.slang` does not compile on its declared target (`-target hlsl`):
the forward-declaration + definition pair `struct ForwardDeclared; struct ForwardDeclared { int x; }`
is rejected with `error[E30200]: conflicting declaration` / `error[E39999]: ambiguous reference`
(exit 255). The failure was masked locally because FileCheck is absent, so `verify` reports the
test as `ignored` rather than running it. The root cause is the compiler/doc drift captured in
finding `types-struct-no-body-forward-decl-conflict` (see `## Doc gaps observed`). The test is
added to `_meta/expected-failures.txt` pending triage of that finding; do not silently delete it.
